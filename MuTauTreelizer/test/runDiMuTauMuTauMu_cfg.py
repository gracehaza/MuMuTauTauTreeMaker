...
process = cms.Process("DiMuonTauMuTauMuTreelizer")
process.load("FWCore.MessageService.MessageLogger_cfi")

########## Please specify if you are running on data (0) or MC (1) in the command line: #########################
########### eg: cmsRun runDiMuTauMuTauMu_cfg.py isMC=1 ###############
##########################################################################

if options.isMC == 1:
    print " ****** we will run on sample of: MC ******"
    process.load("MuMuTauTauTreeMaker.MuTauTreelizer.DiMuTauMuTauMuSelectorMC_cfi")

else:
    print " ****** we will run on sample of: data ******"
    process.load("MuMuTauTauTreeMaker.MuTauTreelizer.DiMuTauMuTauMuSelector_cfi")
...
process.treelizer = cms.Sequence(
        ...
        process.JetSelector*
        process.MuMuTauMuTauMuAnalyzer
)
...
process.TFileService = cms.Service("TFileService",
        fileName =  cms.string('MuMuTauMuTauMuTreelization.root')
)
